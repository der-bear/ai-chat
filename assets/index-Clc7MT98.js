...
`)?"":`
`)+d.raw,f.text+=`
`+d.text,this.inlineQueue.pop(),this.inlineQueue.at(-1).src=f.text):c.push(d);continue}if(l){let f="Infinite loop on byte: "+l.charCodeAt(0);if(this.options.silent){console.error(f);break}else throw new Error(f)}}return this.state.top=!0,c}inline(l,c=[]){return this.inlineQueue.push({src:l,tokens:c}),c}inlineTokens(l,c=[]){let s=l,d=null;if(this.tokens.links){let m=Object.keys(this.tokens.links);if(m.length>0)for(;(d=this.tokenizer.rules.inline.reflinkSearch.exec(s))!=null;)m.includes(d[0].slice(d[0].lastIndexOf("[")+1,-1))&&(s=s.slice(0,d.index)+"["+"a".repeat(d[0].length-2)+"]"+s.slice(this.tokenizer.rules.inline.reflinkSearch.lastIndex))}for(;(d=this.tokenizer.rules.inline.anyPunctuation.exec(s))!=null;)s=s.slice(0,d.index)+"++"+s.slice(this.tokenizer.rules.inline.anyPunctuation.lastIndex);for(;(d=this.tokenizer.rules.inline.blockSkip.exec(s))!=null;)s=s.slice(0,d.index)+"["+"a".repeat(d[0].length-2)+"]"+s.slice(this.tokenizer.rules.inline.blockSkip.lastIndex);let y=!1,f="";for(;l;){y||(f=""),y=!1;let m;if(this.options.extensions?.inline?.some(S=>(m=S.call({lexer:this},l,c))?(l=l.substring(m.raw.length),c.push(m),!0):!1))continue;if(m=this.tokenizer.escape(l)){l=l.substring(m.raw.length),c.push(m);continue}if(m=this.tokenizer.tag(l)){l=l.substring(m.raw.length),c.push(m);continue}if(m=this.tokenizer.link(l)){l=l.substring(m.raw.length),c.push(m);continue}if(m=this.tokenizer.reflink(l,this.tokens.links)){l=l.substring(m.raw.length);let S=c.at(-1);m.type==="text"&&S?.type==="text"?(S.raw+=m.raw,S.text+=m.text):c.push(m);continue}if(m=this.tokenizer.emStrong(l,s,f)){l=l.substring(m.raw.length),c.push(m);continue}if(m=this.tokenizer.codespan(l)){l=l.substring(m.raw.length),c.push(m);continue}if(m=this.tokenizer.br(l)){l=l.substring(m.raw.length),c.push(m);continue}if(m=this.tokenizer.del(l)){l=l.substring(m.raw.length),c.push(m);continue}if(m=this.tokenizer.autolink(l)){l=l.substring(m.raw.length),c.push(m);continue}if(!this.state.inLink&&(m=this.tokenizer.url(l))){l=l.substring(m.raw.length),c.push(m);continue}let v=l;if(this.options.extensions?.startInline){let S=1/0,E=l.slice(1),k;this.options.extensions.startInline.forEach(C=>{k=C.call({lexer:this},E),typeof k=="number"&&k>=0&&(S=Math.min(S,k))}),S<1/0&&S>=0&&(v=l.substring(0,S+1))}if(m=this.tokenizer.inlineText(v)){l=l.substring(m.raw.length),m.raw.slice(-1)!=="_"&&(f=m.raw.slice(-1)),y=!0;let S=c.at(-1);S?.type==="text"?(S.raw+=m.raw,S.text+=m.text):c.push(m);continue}if(l){let S="Infinite loop on byte: "+l.charCodeAt(0);if(this.options.silent){console.error(S);break}else throw new Error(S)}}return c}},Zo=class{options;parser;constructor(u){this.options=u||In}space(u){return""}code({text:u,lang:l,escaped:c}){let s=(l||"").match(ze.notSpaceStart)?.[0],d=u.replace(ze.endingNewline,"")+`
...
`}strong({tokens:u}){return`<strong>${this.parser.parseInline(u)}</strong>`}em({tokens:u}){return`<em>${this.parser.parseInline(u)}</em>`}codespan({text:u}){return`<code>${Ct(u,!0)}</code>`}br(u){return"<br>"}del({tokens:u}){return`<del>${this.parser.parseInline(u)}</del>`}link({href:u,title:l,tokens:c}){let s=this.parser.parseInline(c),d=Mh(u);if(d===null)return s;u=d;let y='<a href="'+u+'"';return l&&(y+=' title="'+Ct(l)+'"'),y+=">"+s+"</a>",y}image({href:u,title:l,text:c,tokens:s}){s&&(c=this.parser.parseInline(s,this.parser.textRenderer));let d=Mh(u);if(d===null)return Ct(c);u=d;let y=`<img src="${u}" alt="${c}"`;return l&&(y+=` title="${Ct(l)}"`),y+=">",y}text(u){return"tokens"in u&&u.tokens?this.parser.parseInline(u.tokens):"escaped"in u&&u.escaped?u.text:Ct(u.text)}},kr=class{strong({text:u}){return u}em({text:u}){return u}codespan({text:u}){return u}del({text:u}){return u}html({text:u}){return u}text({text:u}){return u}link({text:u}){return""+u}image({text:u}){return""+u}br(){return""}},zt=class wr{options;renderer;textRenderer;constructor(l){this.options=l||In,this.options.renderer=this.options.renderer||new Zo,this.renderer=this.options.renderer,this.renderer.options=this.options,this.renderer.parser=this,this.textRenderer=new kr}static parse(l,c){return new wr(c).parse(l)}static parseInline(l,c){return new wr(c).parseInline(l)}parse(l,c=!0){let s="";for(let d=0;d<l.length;d++){let y=l[d];if(this.options.extensions?.renderers?.[y.type]){let m=y,v=this.options.extensions.renderers[m.type].call({parser:this},m);if(v!==!1||!["space","hr","heading","code","table","blockquote","list","html","def","paragraph","text"].includes(m.type)){s+=v||"";continue}}let f=y;switch(f.type){case"space":{s+=this.renderer.space(f);continue}case"hr":{s+=this.renderer.hr(f);continue}case"heading":{s+=this.renderer.heading(f);continue}case"code":{s+=this.renderer.code(f);continue}case"table":{s+=this.renderer.table(f);continue}case"blockquote":{s+=this.renderer.blockquote(f);continue}case"list":{s+=this.renderer.list(f);continue}case"html":{s+=this.renderer.html(f);continue}case"def":{s+=this.renderer.def(f);continue}case"paragraph":{s+=this.renderer.paragraph(f);continue}case"text":{let m=f,v=this.renderer.text(m);for(;d+1<l.length&&l[d+1].type==="text";)m=l[++d],v+=`
`+this.renderer.text(m);c?s+=this.renderer.paragraph({type:"paragraph",raw:v,text:v,tokens:[{type:"text",raw:v,text:v,escaped:!0}]}):s+=v;continue}default:{let m='Token with "'+f.type+'" type was not found.';if(this.options.silent)return console.error(m),"";throw new Error(m)}}}return s}parseInline(l,c=this.renderer){let s="";for(let d=0;d<l.length;d++){let y=l[d];if(this.options.extensions?.renderers?.[y.type]){let m=this.options.extensions.renderers[y.type].call({parser:this},y);if(m!==!1||!["escape","html","link","image","strong","em","codespan","br","del","text"].includes(y.type)){s+=m||"";continue}}let f=y;switch(f.type){case"escape":{s+=c.text(f);break}case"html":{s+=c.html(f);break}case"link":{s+=c.link(f);break}case"image":{s+=c.image(f);break}case"strong":{s+=c.strong(f);break}case"em":{s+=c.em(f);break}case"codespan":{s+=c.codespan(f);break}case"br":{s+=c.br(f);break}case"del":{s+=c.del(f);break}case"text":{s+=c.text(f);break}default:{let m='Token with "'+f.type+'" type was not found.';if(this.options.silent)return console.error(m),"";throw new Error(m)}}}return s}},jo=class{options;block;constructor(u){this.options=u||In}static passThroughHooks=new Set(["preprocess","postprocess","processAllTokens"]);preprocess(u){return u}postprocess(u){return u}processAllTokens(u){return u}provideLexer(){return this.block?Bt.lex:Bt.lexInline}provideParser(){return this.block?zt.parse:zt.parseInline}},Rm=class{defaults=Sr();options=this.setOptions;parse=this.parseMarkdown(!0);parseInline=this.parseMarkdown(!1);Parser=zt;Renderer=Zo;TextRenderer=kr;Lexer=Bt;Tokenizer=Xo;Hooks=jo;constructor(...u){this.use(...u)}walkTokens(u,l){let c=[];for(let s of u)switch(c=c.concat(l.call(this,s)),s.type){case"table":{let d=s;for(let y of d.header)c=c.concat(this.walkTokens(y.tokens,l));for(let y of d.rows)for(let f of y)c=c.concat(this.walkTokens(f.tokens,l));break}case"list":{let d=s;c=c.concat(this.walkTokens(d.items,l));break}default:{let d=s;this.defaults.extensions?.childTokens?.[d.type]?this.defaults.extensions.childTokens[d.type].forEach(y=>{let f=d[y].flat(1/0);c=c.concat(this.walkTokens(f,l))}):d.tokens&&(c=c.concat(this.walkTokens(d.tokens,l)))}}return c}use(...u){let l=this.defaults.extensions||{renderers:{},childTokens:{}};return u.forEach(c=>{let s={...c};if(s.async=this.defaults.async||s.async||!1,c.extensions&&(c.extensions.forEach(d=>{if(!d.name)throw new Error("extension name required");if("renderer"in d){let y=l.renderers[d.name];y?l.renderers[d.name]=function(...f){let m=d.renderer.apply(this,f);return m===!1&&(m=y.apply(this,f)),m}:l.renderers[d.name]=d.renderer}if("tokenizer"in d){if(!d.level||d.level!=="block"&&d.level!=="inline")throw new Error("extension level must be 'block' or 'inline'");let y=l[d.level];y?y.unshift(d.tokenizer):l[d.level]=[d.tokenizer],d.start&&(d.level==="block"?l.startBlock?l.startBlock.push(d.start):l.startBlock=[d.start]:d.level==="inline"&&(l.startInline?l.startInline.push(d.start):l.startInline=[d.start]))}"childTokens"in d&&d.childTokens&&(l.childTokens[d.name]=d.childTokens)}),s.extensions=l),c.renderer){let d=this.defaults.renderer||new Zo(this.defaults);for(let y in c.renderer){if(!(y in d))throw new Error(`renderer '${y}' does not exist`);if(["options","parser"].includes(y))continue;let f=y,m=c.renderer[f],v=d[f];d[f]=(...S)=>{let E=m.apply(d,S);return E===!1&&(E=v.apply(d,S)),E||""}}s.renderer=d}if(c.tokenizer){let d=this.defaults.tokenizer||new Xo(this.defaults);for(let y in c.tokenizer){if(!(y in d))throw new Error(`tokenizer '${y}' does not exist`);if(["options","rules","lexer"].includes(y))continue;let f=y,m=c.tokenizer[f],v=d[f];d[f]=(...S)=>{let E=m.apply(d,S);return E===!1&&(E=v.apply(d,S)),E}}s.tokenizer=d}if(c.hooks){let d=this.defaults.hooks||new jo;for(let y in c.hooks){if(!(y in d))throw new Error(`hook '${y}' does not exist`);if(["options","block"].includes(y))continue;let f=y,m=c.hooks[f],v=d[f];jo.passThroughHooks.has(y)?d[f]=S=>{if(this.defaults.async)return Promise.resolve(m.call(d,S)).then(k=>v.call(d,k));let E=m.call(d,S);return v.call(d,E)}:d[f]=(...S)=>{let E=m.apply(d,S);return E===!1&&(E=v.apply(d,S)),E}}s.hooks=d}if(c.walkTokens){let d=this.defaults.walkTokens,y=c.walkTokens;s.walkTokens=function(f){let m=[];return m.push(y.call(this,f)),d&&(m=m.concat(d.call(this,f))),m}}this.defaults={...this.defaults,...s}}),this}setOptions(u){return this.defaults={...this.defaults,...u},this}lexer(u,l){return Bt.lex(u,l??this.defaults)}parser(u,l){return zt.parse(u,l??this.defaults)}parseMarkdown(u){return(l,c)=>{let s={...c},d={...this.defaults,...s},y=this.onError(!!d.silent,!!d.async);if(this.defaults.async===!0&&s.async===!1)return y(new Error("marked(): The async option was set to true by an extension. Remove async: false from the parse options object to return a Promise."));if(typeof l>"u"||l===null)return y(new Error("marked(): input parameter is undefined or null"));if(typeof l!="string")return y(new Error("marked(): input parameter is of type "+Object.prototype.toString.call(l)+", string expected"));d.hooks&&(d.hooks.options=d,d.hooks.block=u);let f=d.hooks?d.hooks.provideLexer():u?Bt.lex:Bt.lexInline,m=d.hooks?d.hooks.provideParser():u?zt.parse:zt.parseInline;if(d.async)return Promise.resolve(d.hooks?d.hooks.preprocess(l):l).then(v=>f(v,d)).then(v=>d.hooks?d.hooks.processAllTokens(v):v).then(v=>d.walkTokens?Promise.all(this.walkTokens(v,d.walkTokens)).then(()=>v):v).then(v=>m(v,d)).then(v=>d.hooks?d.hooks.postprocess(v):v).catch(y);try{d.hooks&&(l=d.hooks.preprocess(l));let v=f(l,d);d.hooks&&(v=d.hooks.processAllTokens(v)),d.walkTokens&&this.walkTokens(v,d.walkTokens);let S=m(v,d);return d.hooks&&(S=d.hooks.postprocess(S)),S}catch(v){return y(v)}}}onError(u,l){return c=>{if(c.message+=`
Please report this to https://github.com/markedjs/marked.`,u){let s="<p>An error occurred:</p><pre>"+Ct(c.message+"",!0)+"</pre>";return l?Promise.resolve(s):s}if(l)return Promise.reject(c);throw c}}},On=new Rm;function ae(u,l){return On.parse(u,l)}ae.options=ae.setOptions=function(u){return On.setOptions(u),ae.defaults=On.defaults,zh(ae.defaults),ae};ae.getDefaults=Sr;ae.defaults=In;ae.use=function(...u){return On.use(...u),ae.defaults=On.defaults,zh(ae.defaults),ae};ae.walkTokens=function(u,l){return On.walkTokens(u,l)};ae.parseInline=On.parseInline;ae.Parser=zt;ae.parser=zt.parse;ae.Renderer=Zo;ae.TextRenderer=kr;ae.Lexer=Bt;ae.lexer=Bt.lex;ae.Tokenizer=Xo;ae.Hooks=jo;ae.parse=ae;ae.options;ae.setOptions;ae.use;ae.walkTokens;ae.parseInline;zt.parse;Bt.lex;const Jh=new ae.Renderer;Jh.heading=u=>`<strong>${u.text}</strong>`;ae.setOptions({renderer:Jh,gfm:!0,breaks:!0});function xm(u){try{const c=new DOMParser().parseFromString(u,"text/html"),s=new Set(["strong","em","b","i","p","br","ul","ol","li","table","thead","tbody","tr","th","td","a","code","span"]),d={a:new Set(["href","title","target","rel"]),td:new Set(["colspan","rowspan"]),th:new Set(["colspan","rowspan"]),span:new Set([]),code:new Set([])},y=f=>{if(f.nodeType===Node.COMMENT_NODE){f.parentNode?.removeChild(f);return}if(f.nodeType===Node.ELEMENT_NODE){const m=f,v=m.tagName.toLowerCase();if(!s.has(v)){const S=c.createTextNode(m.textContent||"");m.parentNode?.replaceChild(S,m);return}[...m.attributes].forEach(S=>{const E=S.name.toLowerCase();if(E.startsWith("on")){m.removeAttribute(S.name);return}const k=d[v];if(k&&!k.has(E)){m.removeAttribute(S.name);return}if(v==="a"&&E==="href"){const C=S.value.trim();/^\s*javascript:/i.test(C)?m.removeAttribute(S.name):(m.setAttribute("rel","noopener noreferrer"),m.getAttribute("target")||m.setAttribute("target","_blank"))}})}f.childNodes&&[...f.childNodes].forEach(y)};return[...c.body?.childNodes||[]].forEach(y),c.body?.innerHTML||""}catch{return u}}const Nm=({content:u})=>{const l=c=>{try{const s=c.trim(),d=ae.parse(s),y=typeof d=="string"?d:s.replace(/\n/g,"<br>");return{__html:xm(y)}}catch(s){return console.error("Markdown rendering error:",s),{__html:c.replace(/\n/g,"<br>")}}};return x.jsx("div",{className:"markdown-content",dangerouslySetInnerHTML:l(u)})},Mm=({actions:u,onActionClick:l,disabled:c=!1,selectedActionId:s})=>{const[d,y]=ne.useState(s);if(!u||u.length===0)return null;const f=v=>{y(v.id),l(v)},m=s??d;return x.jsx("div",{className:"ai-chat__suggested-actions",children:x.jsx("div",{className:"ai-chat__suggested-actions-container",children:u.map(v=>{const S=m===v.id;return x.jsx("button",{className:`ai-chat__tool-pill ${S?"ai-chat__tool-pill--selected":""}`,onClick:()=>f(v),disabled:c,children:v.text},v.id)})})})},Om=({onFileUpload:u,accept:l=".pdf,.doc,.docx,.txt,.json,.xml,.csv,.xlsx,.xls",maxSize:c=5*1024*1024})=>{const[s,d]=ne.useState(!1),[y,f]=ne.useState(!1),m=ne.useRef(null),v=q=>{q.preventDefault(),d(!0)},S=q=>{q.preventDefault(),d(!1)},E=q=>{q.preventDefault(),d(!1);const ie=Array.from(q.dataTransfer.files);ie.length>0&&C(ie[0])},k=q=>{const ie=q.target.files;ie&&ie.length>0&&C(ie[0])},C=async q=>{if(q.size>c){alert(`File too large. Maximum size is ${c/1024/1024}MB`);return}f(!0);try{const ie=await B(q);u(q,ie)}catch{alert("Error reading file. Please try a different format.")}finally{f(!1)}},B=q=>new Promise((ie,_e)=>{const Ee=new FileReader;Ee.onload=ce=>{const be=ce.target?.result;ie(be)},Ee.onerror=()=>_e(new Error("Failed to read file")),q.type==="application/json"||q.name.endsWith(".json")||q.type.includes("text")||q.name.endsWith(".txt")||q.name.endsWith(".csv"),Ee.readAsText(q)}),_=()=>{m.current?.click()};return x.jsx("div",{className:"file-upload",children:x.jsxs("div",{className:`file-upload__dropzone ${s?"file-upload__dropzone--dragging":""} ${y?"file-upload__dropzone--processing":""}`,onDragOver:v,onDragLeave:S,onDrop:E,onClick:_,children:[x.jsx("input",{ref:m,type:"file",accept:l,onChange:k,style:{display:"none"}}),y?x.jsxs("div",{className:"file-upload__processing",children:[x.jsx("div",{className:"file-upload__spinner"}),x.jsx("p",{children:"Processing file..."})]}):x.jsxs("div",{className:"file-upload__content",children:[x.jsx("div",{className:"file-upload__icon",children:x.jsx("i",{className:"cil-cloud-upload"})}),x.jsx("p",{children:"Drag & drop files here or click to browse"}),x.jsx("div",{className:"file-upload__formats",children:x.jsx("small",{children:"Supported formats: CSV, JSON, XLS"})})]})]})})},Im=({messages:u,isTyping:l,conversationId:c,onSuggestedActionClick:s,onFileUpload:d})=>{const y=ne.useRef(null),f=()=>{y.current?.scrollIntoView({behavior:"smooth"})};ne.useEffect(()=>{f()},[u,l]);const m=v=>{if(v.startsWith("{")||v.startsWith("["))try{const S=JSON.parse(v);return x.jsx("pre",{style:{background:"var(--ai-chat-hover)",padding:"8px",borderRadius:"4px",fontSize:"12px",overflow:"auto"},children:JSON.stringify(S,null,2)})}catch{}return v.split(`
`).map((S,E)=>x.jsxs(Ep.Fragment,{children:[S,E<v.split(`
`).length-1&&x.jsx("br",{})]},E))};return x.jsxs("main",{className:"ai-chat__messages",children:[u.length>0&&c&&x.jsx("div",{className:"ai-chat__session-header",children:x.jsx("span",{className:"ai-chat__session-id",children:c})}),u.map((v,S)=>x.jsxs("div",{className:`ai-chat__message ${v.sender==="user"?"ai-chat__message--user":""}`,children:[x.jsx("div",{className:`ai-chat__avatar ai-chat__avatar--${v.sender}`,children:x.jsx("i",{className:`cil-${v.sender==="user"?"user":"star"}`})}),x.jsxs("div",{className:"ai-chat__content",children:[x.jsxs("div",{className:`ai-chat__bubble ai-chat__bubble--${v.sender}`,children:[v.sender==="assistant"?x.jsx(Nm,{content:v.text}):m(v.text),v.fileUpload&&d&&S===u.length-1&&x.jsx(Om,{onFileUpload:d})]}),v.agentUsed&&x.jsxs("div",{className:"ai-chat__agent-indicator",children:["via ",v.agentUsed," agent"]}),v.suggestedActions&&v.suggestedActions.length>0&&s&&x.jsx(Mm,{actions:v.suggestedActions,onActionClick:s,disabled:l||S<u.length-1})]})]},v.id)),l&&x.jsxs("div",{className:"ai-chat__message",children:[x.jsx("div",{className:"ai-chat__avatar ai-chat__avatar--assistant",children:x.jsx("i",{className:"cil-star"})}),x.jsx("div",{className:"ai-chat__content",children:x.jsx("div",{className:"ai-chat__bubble ai-chat__bubble--assistant",children:x.jsxs("div",{className:"ai-chat__typing",children:[x.jsx("span",{className:"ai-chat__typing-dot"}),x.jsx("span",{className:"ai-chat__typing-dot"}),x.jsx("span",{className:"ai-chat__typing-dot"})]})})})]}),x.jsx("div",{ref:y})]})},Um=({onSendMessage:u,isTyping:l})=>{const[c,s]=ne.useState(""),[d,y]=ne.useState(!1),f=ne.useRef(null),m=C=>{s(C.target.value),v(C.target)},v=C=>{const B=C.scrollTop;C.style.height="auto";const _=160,q=40,ie=Math.min(C.scrollHeight,_);C.style.height=`${ie}px`,C.style.overflowY=C.scrollHeight>_?"auto":"hidden",y(ie>q+5),C.scrollTop=B},S=()=>{const C=c.trim();!C||l||(u(C),s(""),y(!1),f.current&&(f.current.style.height="auto"))},E=C=>{C.key==="Enter"&&!C.shiftKey&&(C.preventDefault(),S())},k=!c.trim()||l;return x.jsx("footer",{className:"ai-chat__footer",children:x.jsxs("div",{className:`ai-chat__input-group ${d?"ai-chat__input-group--multiline":""}`,children:[x.jsx("div",{className:"ai-chat__input-wrapper",children:x.jsx("textarea",{ref:f,className:"ai-chat__input",placeholder:"Type your message...",rows:1,value:c,onChange:m,onKeyDown:E})}),x.jsx("button",{className:"ai-chat__send",type:"button",disabled:k,onClick:S,children:x.jsx("i",{className:"cil-send"})})]})})};function $h({items:u,searchPlaceholder:l="Search...",emptyMessage:c="No items found",onItemClick:s,itemTemplate:d,searchFilter:y,className:f=""}){const[m,v]=ne.useState(""),S=ne.useMemo(()=>{if(!m.trim())return u;if(y)return u.filter(C=>y(C,m));const k=m.toLowerCase();return u.filter(C=>String(C.id).toLowerCase().includes(k))},[u,m,y]),E=ne.useMemo(()=>S.length?S.some(C=>C.category)?S.reduce((C,B)=>{const _=B.category||"Other";return C[_]||(C[_]=[]),C[_].push(B),C},{}):{All:S}:{},[S]);return x.jsx("div",{className:`ai-chat__history ${f}`,children:x.jsxs("div",{className:"ai-chat__history-content",children:[x.jsx("div",{className:"ai-chat__history-search-sticky",children:x.jsxs("div",{className:"ai-chat__history-search",children:[x.jsx("i",{className:"cil-search ai-chat__history-search-icon"}),x.jsx("input",{type:"text",className:"ai-chat__history-search-input",placeholder:l,value:m,onChange:k=>v(k.target.value)})]})}),x.jsx("div",{className:"ai-chat__history-list",children:Object.keys(E).length===0?x.jsx("div",{className:"ai-chat__history-empty",children:c}):Object.entries(E).map(([k,C])=>x.jsxs("div",{className:"ai-chat__category-section",children:[k!=="All"&&x.jsxs("div",{className:"ai-chat__category-header",children:[k," (",C.length,")"]}),C.map(B=>d(B,()=>s?.(B)))]},k))})]})})}const _m=({conversations:u,currentConversationId:l,onConversationSelect:c})=>{const s=d=>{const y=new Date(d),m=new Date().getTime()-y.getTime(),v=Math.floor(m/(1e3*60*60*24));return v===0?"Today":v===1?"Yesterday":v<7?`${v}d ago`:y.toLocaleDateString()};return x.jsx($h,{items:u,title:"LeadExec Copilot",icon:"cil-star",searchPlaceholder:"Search chat sessions...",emptyMessage:u.length===0?"No conversations yet":"No matching conversations",onItemClick:d=>c(d.id),searchFilter:(d,y)=>d.title.toLowerCase().includes(y.toLowerCase()),itemTemplate:(d,y)=>x.jsx("button",{className:`ai-chat__history-item ${d.id===l?"ai-chat__history-item--active":""}`,onClick:y,"aria-label":`${d.title} - ${s(d.lastUpdated)}`,children:x.jsxs("div",{className:"ai-chat__history-item-content",children:[x.jsx("span",{className:"ai-chat__history-item-title",children:d.title}),x.jsx("span",{className:"ai-chat__history-item-date",children:s(d.lastUpdated)})]})},d.id)})},Uh=`Advice and answers from the LeadExec Team 
...
✓ All URLs requested, never extracted from instructions`,Fm=`# LeadExec Copilot — Universal Agent Instructions
