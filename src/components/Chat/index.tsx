...
import { ResizeState } from '../../types/chat';

// Import documentation content
import documentationContent from '../../data/documentation.md?raw';
...
export const Chat: React.FC<ChatProps> = ({ className = '' }) => {
  ...
  const [openaiService] = useState(() => {
    ...
    service.setDocumentation(documentationContent);
    return service;
  });
    ...
    try {
      ...
      if (response.toolCalls && response.toolCalls.length > 0) {
        const toolResults = await Promise.all(
          response.toolCalls.map(toolCall => openaiService.executeToolCall(toolCall))
        );
        
        // Combine tool results with the response
